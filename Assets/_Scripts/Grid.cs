...
public class Grid : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        {
            Debug.LogError("NavMeshSurface no encontrado en el objeto " + gameObject.name);
            return;
        }
        if (terrainMaterial == null)
        {
            Debug.LogError("Material de terreno no asignado.");
            return;
        }
        if (edgeMaterial == null)
        {
            Debug.LogError("Material de borde no asignado.");
            return;
        }
    ...
    void CreateNavMeshModifiers(Cell[,] grid)
    {
        for (int y = 0; y < size; y++)
        {
            for (int x = 0; x < size; x++)
            {
                ...
                if (cell.isWater)
                {
                    GameObject waterCell = new GameObject("WaterCell");
                    waterCell.transform.position = new Vector3(x, 0, y);
                    var navMeshModifier = waterCell.AddComponent<NavMeshModifierVolume>();
                    navMeshModifier.center = Vector3.zero;
                    navMeshModifier.size = new Vector3(1, 1, 1);
                    navMeshModifier.area = NavMesh.GetAreaFromName("Not Walkable");
                    waterCell.transform.parent = transform;
                }
            }
        }
    }
    ...
    void PositionPlayer()
    {
        ...
        for (int y = 0; y < size; y++)
        {
            for (int x = 0; x < size; x++)
            {
                ...
                if (!cell.isWater)
                {
                    player.transform.position = new Vector3(x, 1, y); // Posiciona el jugador un poco sobre el terreno
                    return; // Sale de la función después de encontrar la primera celda válida
                }
            }
        }
    }
